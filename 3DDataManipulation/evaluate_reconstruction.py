...
class meshObj(object):
    ...
    def load_from_obj(self, path, MeshNum):
        ...
        self.landmarks_3D = np.loadtxt("/".join(path.split("/")[:-1]) + "/" + 'VertexLandmarks' + str(MeshNum) + '.txt',
                                       dtype='int32').flatten()

        # check for the number of  landmarks to be 51
        if self.landmarks_3D.shape[0] != 51:
            print("File Format Error: " + 'VertexLandmarks' + str(
                MeshNum) + '.txt' + ' file does not have exactly 51 landmarks.')
            quit()
...
def align_source_to_target(source, target, scale=False):
    ...
    R = np.matmul(np.matmul(V.transpose(), np.diag([1, 1, np.linalg.det(np.matmul(V.transpose(), U.transpose()))])),
                  U.transpose())  # calculate rotation matrix (exclude mirror symmetry)
...
def point_to_face_distance(source, target, f_t, nbrs):
    with warnings.catch_warnings():
        ...
        _, idx, dist = get_closest_point(source, target,
                                         nbrs)  # get closest vertex on target mesh for each vertex of source mesh
        ...
        for i in range(len(idx)):
            ...
            out_idx = ((bary1 <= 0) | (bary1 >= 1) | (bary2 <= 0) | (bary2 >= 1) | (bary3 <= 0) | (bary3 >= 1) | (
                        np.abs(d) < 1e-16))
...
def eval_mesh(test_mesh, res_dir, ref_dir, reffile_prefix='mesh', resfile_prefix='pred'):
    ...
    reconstruct_shape, r, t, s = icp(reconstruct_shape, gt_shape, landmark_s_4crop, landmark_t_4crop, nbrsT_4crop,
                                     max_iter=1)
    ...
    for (a, b, c) in mesh_s.faces:
        if (a in indxSel) and (b in indxSel) and (c in indxSel):
            reconFaceCrop.append(
                [np.where(indxSel == a)[0][0], np.where(indxSel == b)[0][0], np.where(indxSel == c)[0][0]])
    ...
    errorS2T, errorT2S, RR, tt, ss = reconstruct_distance(reconstruct_shape_crop, gt_shape, f_s, f_t, landmark_s,
                                                          landmark_t)
